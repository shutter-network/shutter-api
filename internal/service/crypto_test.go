...
func TestWithEVM(t *testing.T) {
	...
	mOne := shs.LogPredicate{
		LogValueRef:    shs.LogValueRef{Offset: 1},
		ValuePredicate: shs.ValuePredicate{Op: shs.BytesEq, ByteArgs: [][]byte{usecase.Align(one.Bytes())}},
	}
	two := "two"
	jTwo := fmt.Sprintf(`{"name": "two", "op": "eq", "bytes": "%v"}`, hexutil.Encode(usecase.Align(crypto.Keccak256([]byte("two")))))
	mTwo := shs.LogPredicate{
		LogValueRef: shs.LogValueRef{Offset: 2},
		ValuePredicate: shs.ValuePredicate{Op: shs.BytesEq, ByteArgs: [][]byte{
			usecase.Align(crypto.Keccak256([]byte("two"))),
		}},
	}
	...
	mThree := shs.LogPredicate{
		...
		ValuePredicate: shs.ValuePredicate{Op: shs.BytesEq, ByteArgs: [][]byte{usecase.Align(three[:])}},
	}
